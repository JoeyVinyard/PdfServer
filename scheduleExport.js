...
exports.exportSchedule = (data) => {
  ...
  let baseTable = "<html><body>{{TABLE}}</body><style>"
                ...
                + "th{border-bottom:1px solid black; border-right:1px solid black; border-left:1px solid black; font-size: 11px}"
                + ".label {font-size: 16px;}"
                + ".name{min-width: 60px;}"
                + ".name p{font-size: 11px; padding-right: 5px}"
                + ".left{border-right:1px solid black}"
                + ".right{border-left:1px solid black}"
                + ".hbuffer {min-width: 80px;}"
                + ".hbuffer p{font-size: 11px; margin-right: 2px;}"
